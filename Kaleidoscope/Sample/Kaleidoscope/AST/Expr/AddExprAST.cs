...
public class AddExprAST : ExprAST
{
    public CodegenLLVMSharpEleNode LHS;
    public CodegenLLVMSharpEleNode RHS;

    public AddExprAST(CodegenLLVMSharpEleNode LHS, CodegenLLVMSharpEleNode RHS)
    {
        this.LHS = LHS;
        this.RHS = RHS;
    }

    public override unsafe PreAddInst Codegen()
    {
        ...
        return new PreAddInst(lhsValue, rhsValue);
    }

    public override string ToStr(Printer printer)
    {
        var result = "";
        result += "AddExpr {".ToStr(printer) + "\n";
